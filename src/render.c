...
/*   Updated: 2025/06/22 20:27:36 by kaisuzuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	color_put(t_img *img, int x, int y, t_iterator it)
{
	int	color;
	int	r;
	int	g;
...
void	render(t_data *data, int max_iter)
{
	...
	while (y < data->win.height)
	{
		...
		{
			it.iter = data->f(pixel_to_complex(&data->win, x, y), data->z0,
					max_iter);
			color_put(&data->img, x, y, it);
			x++;
		}
		y++;
	}
	mlx_put_image_to_window(data->win.mlx, data->win.mlx_win, data->img.img, 0,
		0);
}
